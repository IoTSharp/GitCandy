...
对应 `ROADMAP.md`：M9 `#101/#106-#109`、M10、M11、M12、M13。

本文比较 GitHub、GitLab、Gitea/Forgejo 和 Gitee 的代码浏览、Issue、Pull Request / Merge Request、代码评审和合并治理能力，并据此定义 GitCandy 第一阶段迁移完成后的产品优先级。本文只规划产品和架构边界，不表示对应能力已经实现。

## 1. 结论

GitCandy 眼下最值得补的不是内置 CI、Packages 或复杂项目管理，而是以下连续闭环：

1. **先完成代码工作区**：真实仓库创建/导入、tree、blob、raw、commit、diff、blame、compare，以及可引用的代码行和代码片段。
2. **再完成 Issue**：仓库内问题跟踪、评论、Markdown 代码块、标签、里程碑、负责人、引用和通知。
3. **再完成 Pull Request**：先支持同仓库分支，具备 draft、commits、files changed、行内评论、review、approval、冲突判断和 merge/squash。
4. **最后补合并治理与集成**：PAT、branch protection、CODEOWNERS、commit status/check、webhook、通知中心、审计和 release。
...
| --- | --- | --- | --- | --- | --- |
| 代码浏览 | tree/blob/raw、commit、compare、permalink、行号引用 | repository/files、commit、compare、blame，与 MR 紧密联动 | 轻量 tree/blob/commit/diff，部署成本低 | 代码浏览、提交和 PR 文件改动 | M9 立即补齐；代码行永久链接和大文件边界是 PR 前置条件 |
| Issue | 灵活 Issue、子 Issue/依赖、labels、milestones、assignee、订阅、模板 | Issue/work item、labels、milestones、due date、thread、boards/epics | Issue/PR 共用 labels、templates、自动引用，轻量直接 | Issue、负责人/协作者、标签、里程碑 | M11 先做仓库级核心，不先做 board/epic/iteration |
| PR / MR | Conversation、Commits、Checks、Files changed、draft、临时 refs | description、commits、diff、pipeline、mergeability、assignee/reviewer | fork/branch PR、WIP、review、merge，适合轻量自托管 | fork PR、draft、代码评论、squash | M12 先做同仓库分支闭环；跨 fork 在稳定 namespace/fork 完成后接入 |
...
当前活动主线已经能读取 HEAD、commit、branch、tag 快照，但 `RepositoryController.Tree` 仍只重定向到仓库详情页。M9 `#101` 不能仅靠页面样式关闭，必须先由 `#106-#108` 提供真正的读取服务和仓库生命周期。`#109` 是并列的 Git LFS v2 协议切片，不与代码浏览 UI 混成一个实现任务。

建议兼容现有路由，并在 M10 稳定 namespace 后增加规范地址。永久链接必须固定 commit SHA，分支地址可随 HEAD 变化：

```text
/Repository/{repository}/Tree/{ref}/{**path}
/Repository/{repository}/Blob/{ref}/{**path}#L10-L20
/Repository/{repository}/Raw/{ref}/{**path}
/Repository/{repository}/Commit/{sha}
/Repository/{repository}/Compare/{base}...{head}

/{namespace}/{repository}/src/{commitSha}/{**path}#L10-L20
```
...
- M9 `#106-#108`：仓库生命周期、tree/blob/raw、commit/diff/blame/compare、代码行永久链接和边界测试；`#109` 独立补齐 Git LFS batch/upload/download/verify。
- M10：稳定 namespace、历史 alias 和统一 resolver，为跨仓库引用、fork PR 与远程连接提供稳定 ID。
- M11：Issue 核心闭环。
- M12：同仓库分支 PR/review/merge 核心闭环；M10 稳定 namespace 与 `#106` fork 生命周期就绪后补跨 fork PR。
- M13：PAT、webhook、status/check、branch protection、CODEOWNERS、通知、审计、release 和搜索。
