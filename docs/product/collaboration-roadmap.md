...
完成基线：M9-M13。验收入口见 `docs/roadmap/completed-milestones.md`。

本文比较 GitHub、GitLab、Gitea/Forgejo 和 Gitee 的代码浏览、Issue、Pull Request / Merge Request、代码评审和合并治理能力，并记录 GitCandy 的取舍。M9-M13 内容均已落地，后续扩展应保持本文的权限、协议和事件边界。
...
4. **合并治理与集成已完成**：PAT、branch protection、CODEOWNERS、commit status/check、webhook、通知外投、审计、Release、搜索和外部 CI gate 已形成闭环。
...
- M9-M12 已完成并移入完成历史。
- M12.7 建立统一 inbox、Todo 和 Feed 基础；M13 已在其上完成 PR/review/check/release 通知和外部投递。
- M13 已完成 PAT、webhook、status/check、branch protection、CODEOWNERS、审计、Release、搜索和外部 CI 总验收。
