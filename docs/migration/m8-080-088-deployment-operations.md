...
- GitCandy 普通启动会在 Web、SSH、Quartz 和其他 hosted service 前检测并自动应用 pending migrations；`GitCandy --migrate` 保留为只迁移后退出的兼容入口，Compose 不再需要一次性迁移服务。
...
- 自动 migration 用主程序启动检查替代 Compose/安装脚本编排，保持 GitCandy 单进程部署边界；代价是启动新版本本身可能修改 schema。
...
- 启动新版本前必须备份，因为进程会自动应用 pending migrations。迁移失败会阻止服务开始监听；若回滚到旧二进制，必须同时恢复旧数据库 schema 快照，不能只替换程序。
...
- Docker image 构建成功；单个非 root Compose 容器自动完成 migration，readiness/liveness 均为 HTTP 200。
