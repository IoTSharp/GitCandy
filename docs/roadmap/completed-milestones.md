...
```powershell
dotnet build GitCandy.slnx --configuration Release -p:UseArtifactsOutput=true -p:ArtifactsPath=.artifacts/m15-5-final
dotnet test GitCandy.slnx --configuration Release --no-build --no-restore -p:UseArtifactsOutput=true -p:ArtifactsPath=.artifacts/m15-5-final
```

结果：

- Release 构建成功，0 warning、0 error；
- `GitCandy.Data.Tests`：101/101 通过；
- `GitCandy.Tests`：144/144 通过；
- 合计 245/245 通过，无跳过、无失败；
- SQLite、SQL Server、SonnetDB 均确认当前模型与最后 migration 一致。
- JekyllNet 生成 13 个页面和 5 个静态文件；隔离 SQLite host 的 `/help` 在桌面/390px Chrome 下完成搜索、导航、响应式和 console smoke，0 error、0 warning。
...
| 运维与文档 | Docker Compose、Linux systemd、Windows Service、TLS proxy、health checks、OpenTelemetry、migration、`gitcandy.com` 生产部署和恢复演练；随产物发布的版本化 `/help` 帮助中心 |
...
| 编号 | 完成结论 | 证据入口 |
| --- | --- | --- |
| `#169A` | 86 个 Markdown 源逐项登记 owner/audience/public/canonical/archived/version/permalink，current 与 archive 分离 | [M15.5 acceptance](../migration/m15-5-help-center.md)、`HelpDocumentationTests` |
| `#169B-#169D` | 用户协作、管理运维、API/Webhook/MCP/架构共 13 篇 current canonical 文档完成，未实现能力明确标记 unavailable | `/help/current/`、`docs/help/current`、`search-index.json` |
| `#169E` | JekyllNet 0.2.5 local tool、layout、响应式离线主题、代码/表格样式、搜索 metadata 和 asset version 完成 | `.config/dotnet-tools.json`、`docs/help` |
| `#169F` | 主菜单、匿名 `/help`、PathBase、目录 permalink、404、GET/HEAD、cache/ETag/CSP/content type 和路径边界完成 | `HelpEndpointTests`、桌面/390px Chrome smoke |
| `#169G` | build/publish/Docker/Linux/Windows 统一生成并携带 help + manifest，release/container smoke 检查实际 HTTP 与文件 | `GitCandy.csproj`、`Dockerfile`、release workflows |
| `#169H` | inventory/front matter/manifest/search/offline assets 自动检查，Release build/publish 和真实 Chrome 无 console error 验收完成 | [M15.5 acceptance](../migration/m15-5-help-center.md)、`HelpDocumentationTests`、`tools/browser/smoke.mjs` |
...
| 文档主题 | 推荐资料来源 |
| --- | --- |
| 当前安装、升级、备份与回滚 | `/help/current/operations/deployment/`、[deployment guide](../deployment.md)、M8/M12.5 migration records |
...
| 当前用户、管理员和开发者指南 | `/help/current/`；历史依据继续使用本索引和对应 migration records |
