...
- `GitCandy.Data.Tests`：61/61 通过；
- `GitCandy.Tests`：102/102 通过；
- 合计 163/163 通过，无跳过、无失败；
- 源码和测试中未发现 `TODO`、`FIXME`、`HACK` 或已登记的完成项功能缺陷。

本次没有重新执行远程生产部署、真实外部 DNS/TLS、生产备份恢复或跨操作系统 CI。这些需要外部环境的证据以各里程碑文档和 CI 记录为准。M12.6 `#139L` 尚未完成，因此没有归档为完成历史。
...
| 数据 | EF Core + ASP.NET Core Identity、SQLite 默认 provider、SQL Server migration 验证、provider-neutral 数据层 |
...
| 协作 | Issues、评论、Markdown、labels、milestones、assignees、站内 Issue 通知、PR、跨 fork、行内 review、approval、merge/squash |
| 运维 | Docker Compose、Linux systemd、Windows Service、TLS proxy 示例、health checks、OpenTelemetry、migration、发布和恢复演练 |
...
M12.6 不能整体归档，因为生产部署 `#139L` 尚未验收。为保证活动路线图只显示未完成工作，已经闭环的前置任务单独记录在这里：

| 编号 | 完成结论 | 证据入口 |
| --- | --- | --- |
| `#139J` | Host 根据 `GitCandy:Database:Provider` 只注册 SQLite 或 SonnetDB 之一，默认仍为 SQLite；Web 和 OpenSSH 命令路径复用同一 composition root | [database providers](../database-providers.md)、`WebServiceCollectionExtensionsTests` |
| `#139K` | SonnetDB 独立完整 migration、Identity/repository smoke、NULL 唯一索引、事务读取、savepoint 声明、`COALESCE` 投影和 Server build 兼容修复完成 | [database providers](../database-providers.md)、`GitCandySonnetDbSmokeTests`、[production profile](../../deploy/sonnet-vip/README.md) |
...
- Issue inbox 当前只覆盖 M11 Issue 通知。统一 Todo、PR/review/check 通知和 dashboard 属于活动路线图 M12.7/M13，不是 M11 的遗漏。
...
- 一个进行中里程碑只有在所有验收项完成后才能整体迁入本文件；不能只归档其中未完成的生产验收。
