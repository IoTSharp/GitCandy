...
单向 Pull/Push mirror 已使用同一受控 backend。Pull mirror 在 `refs/gitcandy/mirrors/` 隔离区比较远端 refs，正式 Git transport 会隐藏该 namespace，操作结束后清理；启用期间所有本地写入口都拒绝写入。Push mirror 的 `post-receive` 子进程只把每个 mirror/ref 的最新 generation 保存到 `RemoteMirrorRefUpdates`，durable worker 再异步执行远端 fetch/compare/push，因此远端失败不回滚本地 push。初始、schedule、webhook、manual、post-receive 和 recovery 触发合并到每个 mirror 唯一的 `RemoteMirrorJobs` 记录；optimistic concurrency lease 防止重复执行，到期 lease 自动恢复，可重试故障使用指数退避和 jitter。仓库 owner 通过 `/{namespace}/{repository}/settings/mirrors` 暂停、恢复、立即同步、取消、重试和删除。

`GitCandy:Remotes:Jobs` 默认 `MaxConcurrentJobs=2`、`DispatchBatchSize=10`、`MaxAttempts=5`、`LeaseDuration=00:35:00`、`InitialRetryDelay=00:00:15`、`MaximumRetryDelay=00:15:00`、`RetryJitterRatio=0.2`。`LeaseDuration` 必须大于远程 Git `OperationTimeout`；提高并发前评估 Git HTTP/SSH 延迟、磁盘、CPU 和出站带宽。部署仍建议先保持单实例，当前 lease/恢复不能替代完整的多区域协调和灾难恢复方案。

Provider callback 为 `/remote-events/{connectionId}/{provider}`。连接只保存 `env:` 或 `config:` webhook secret reference；GitHub 使用 `X-Hub-Signature-256`，GitLab 使用 `X-Gitlab-Token`，Gitee 使用 `X-Gitee-Token`。生产反向代理应允许最多 1 MiB request body 且不得记录这些 header 或 body。每个真实 provider/tenant 上线前必须 smoke 仓库发现、token scope/expiry/rotation、callback 注册与签名、rename、delete、rate-limit 以及 Pull/Push；fixture 测试不能验证上游应用授权。GitHub App 当前绑定已获取且带到期时间的 installation token，不从 private key 自动换票；GitLab/Gitee OAuth 当前绑定已有 access token，不发起交互式 consent。

数据库与仓库必须一致备份：只恢复数据库可能重放已不存在的本地 ref，只恢复仓库会丢失尚未发送的 pending event。二进制回滚前停止 scheduler 和 push 写入，并恢复升级前同一时间点的数据库、repositories、Data Protection key ring 与 secret 配置；只执行 down migration 会删除 pending/retry 和 provider event receipt 状态。
