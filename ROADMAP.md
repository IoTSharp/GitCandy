...
| 优先级 | 必须继续完成 |
| --- | --- |
| P0 稳定性 | Git HTTP alias 真实客户端稳定性、Issue 并发限流原子性、Linux/Windows 跨平台测试、覆盖率与浏览器回归门禁 |
| P0 基本功能 | Branches/Tags 独立页面与受控管理、Contributors 统计、自助密码恢复/邮件确认、TLS reverse proxy 快速部署、PAT/API auth、deploy key、保护分支、协作审计 |
| P1 研发闭环 | webhook、commit status/check、required checks、CODEOWNERS、PR/review/check 通知、Releases、assets、协作搜索 |
| P2 扩展能力 | 企业身份、remote mirror、Wiki、Packages、内置 runner、LFS locking、代码智能与 Agent Memory |
...
- 代码浏览：tree、blob、raw、commit、diff、blame、compare、archive 已完成；branch/tag ref 已支持读取和 revision 选择，独立 Branches/Tags 页面与受控管理、Contributors 统计在 M12.5 `#139G-#139I` 收口。
...
- 活动主线已经能读取 branch/tag ref 并用于代码页和 PR，但尚未恢复旧版独立 Branches/Tags/Contributors 页面；M9 完成口径只覆盖 ref 读取与 revision 选择，页面和管理闭环由本阶段补齐。
...
| 编号 | 主题 | 验收重点 |
| --- | --- | --- |
| 🚧 #139A | Git HTTP alias 稳定性 | 隔离完整测试中出现的 alias `308` 后 `remote transport reported error`，覆盖连续/并发 clone、fetch、push、协议 v2、响应结束和 reverse proxy；不得用重试掩盖协议缺陷 |
...
| ⬜ #139G | Branches 页面与基础管理 | 为 legacy 与规范 namespace 地址提供分支列表，显示默认分支、tip commit、更新时间及相对默认分支的 ahead/behind；空仓库和 unborn HEAD 明确降级。删除操作要求仓库写权限、antiforgery、默认分支保护、结构化 ref 参数和受控服务；真实 fetch/prune 后结果一致，并为 M13 branch protection 保留统一策略入口 |
| ⬜ #139H | Tags 页面与基础管理 | 为 legacy 与规范 namespace 地址提供 tag 列表，区分 lightweight/annotated tag，显示目标 commit、tagger/date/message 和 archive 入口；删除要求仓库写权限、antiforgery、合法 `refs/tags/*` 校验和受控服务，真实 fetch/prune 后结果一致。signed tag 展示与策略校验仍按 P2 独立推进 |
| ⬜ #139I | Contributors 与仓库统计 | 按指定 branch/tag/commit 聚合 Git author commit 数并展示 contributor 排名，同时恢复 commit、contributor、file、source size 和 repository size 摘要；作者归并规则显式且不自动绑定 Identity、不公开邮箱。遍历数量、输出数量、缓存、超时、取消和大仓库降级可配置，私有仓库权限与非法 revision/path 不泄漏数据 |
...
- 仓库导航可进入 Branches、Tags、Contributors；公开/私有、匿名/可写/管理员、空仓库和非法 ref 均有 MVC 集成测试，legacy 与规范 namespace 地址行为一致。
...
1. 🚧 M12.5：先修 Git HTTP alias 间歇失败和 Issue 并发限流，再按 `#139G -> #139H -> #139I` 补齐 Branches/Tags/Contributors，并完成密码恢复、TLS 快速部署、跨平台 CI、覆盖率与恢复演练。
...
5. ⬜ M16：最后推进 Code Intelligence；首批只做 `#170` schema 与 `#171` 只读增量 ingest，不进入 Git HTTP/SSH 热路径。

明确后置：Wiki、Packages、内置 CI runner、双向 mirror、LFS locking 和 AI 写入能力不阻塞 M12.5-M13，也不得绕过后续建立的 PAT、branch protection、check 和审计边界。
