...
编号规则：路线图编号按优先级和实施顺序排列；编号越小，越应优先推进。同一 Milestone 内按 `#` 任务编号顺序推进。M12.5 使用 `#139A-#139I`，M12.6 使用 `#139J-#139L`，M12.7 使用 `#139M-#139W` 表示插入 M12 与 M13 之间的收口任务；M15.5 使用 `#169A-#169H` 插入 M15 与 M16 之间，避免重编号已经被文档引用的任务。当前没有验收记录的编号默认标为 ⬜。
...
| P2 扩展能力 | 企业身份、remote mirror、帮助中心与文档发布、Wiki、Packages、内置 runner、LFS locking、SonnetDB 文档知识库、全业务 API MCP、代码智能与 Agent Memory |
...
- `Stars` 纳入最小可用的仓库收藏领域能力；`Packages` 先定义可复用的目录页与数据契约，真正的 package registry、上传下载协议、存储、保留策略和签名验证仍须后续独立 Milestone，不能伪装成已可发布制品。
...
| `Packages` | 提供名称搜索、生态/类型与可见性筛选、最近更新排序；目录项展示 package 名、最新版本、类型、来源仓库、可见性、更新时间和进入详情的链接；上传、下载、删除、版本保留等能力只在后续 registry 服务和权限闭环完成后出现 |
...
| ⬜ #139Q | Packages 目录页边界 | 完成目录 ViewModel、筛选/排序/分页、权限与真实空状态；形成 registry 后端非目标文档，不新增假上传接口或第二套对象存储 |
...
7. ⬜ M16：先推进 `#170-#175` 只读代码索引，再以 M15.5 文档执行 `#180-#185` SonnetDB 知识库、文档 MCP 和只读业务 tools；随后完成 `#176-#179` Agent Memory/Explorer/规模报告，并以 `#186-#187` 收口写工具与全部业务 API coverage；全程不进入 Git HTTP/SSH 热路径。
