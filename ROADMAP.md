...
编号规则：路线图编号按优先级和实施顺序排列；编号越小，越应优先推进。同一 Phase 内按 `#` 任务编号顺序推进。当前没有验收记录的编号默认标为 ⬜。
...
- 第一阶段数据库验收门槛为 SQLite 默认可用，并保持 SQL Server schema/migration SQL 路径可行。PostgreSQL 和 SonnetDB 可以保留为可选 provider 扩展，但不替代 SQL Server 可行性要求。
...
├── src/
│   ├── GitCandy.Web/          # ASP.NET Core 10 MVC app
...
如果希望降低第一轮改造成本，也可以先只创建 `src/GitCandy.Web` 一个 SDK-style 项目，把旧目录按原名搬入并替换 API；待测试跑通后再拆 `Core/Data/Git/Ssh`。不要同时重写架构、UI 和协议层。
...
- 以 Phase 10 / P10 为主线，先做只读 ingest、搜索、符号、调用关系、影响分析和 MCP tools。
...
### ⬜ Phase 0 / Milestone 0：基线冻结与迁移保护网
...
### ⬜ Phase 1 / Milestone 1：新 ASP.NET Core 10 MVC 外壳
...
| 编号 | 主题 | 验收重点 |
| --- | --- | --- |
| ⬜ #010 | SDK-style Web 项目 | 创建 `src/GitCandy.Web` ASP.NET Core MVC 项目，目标 `net10.0` |
...
### ⬜ Phase 2 / Milestone 2：配置、日志、缓存、DI 和后台生命周期
...
### 🚧 Phase 3 / Milestone 3：EF Core 数据层迁移
...
- SQLite 是默认本地 provider，SQL Server 是第一阶段必须保持可行的部署路径；两者至少要能生成 migration SQL 并通过 schema smoke tests。
...
| 🚧 #039 | 数据层 smoke tests | 已覆盖 SQLite `EnsureCreated` 和 Identity 用户读写；migration-backed 创建、团队/仓库 CRUD、权限查询、SQL Server migration SQL 待后续，PgSQL/SonnetDB 作为可选 provider 验证 |
...
### ⬜ Phase 5 / Milestone 5：MVC Controllers 和 Razor Views 迁移
...
### ⬜ Phase 6 / Milestone 6：Git Smart HTTP 迁移
...
### ⬜ Phase 7 / Milestone 7：SSH 和后台任务现代化
...
### ⬜ Phase 9 / Milestone 9：迁移稳定后的独立改进
...
### ⬜ Phase 10 / P10：Agent Memory / Codebase Intelligence
...
- ⬜ P10 纳入 GitCandy 应用层产品路线，作为迁移稳定后的重点能力建设。
...
| ⬜ P10 | 代码智能产品能力 | Agent Memory / Codebase Intelligence 完成 schema、ingest、MCP/API、Hybrid Search、Explorer、IDE 接入样例和规模验证 |
...
⬜ P10 的代码智能能力进入第二波实施：等 Git Smart HTTP、SSH/scheduler、部署路径稳定后，再从 ⬜ #150 schema 和 ⬜ #151 ingest 垂直切片开始推进。
...
- ⬜ P10 索引与检索必须严格复用仓库权限，避免私有仓库、未发布分支、代码片段或 Agent 会话被越权读取
